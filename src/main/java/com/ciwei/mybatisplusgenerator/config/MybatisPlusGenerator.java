...
//		gc.setEnableCache(true); // 是否在xml中添加二级缓存配置,默认false
		...
		//如果moduleName不为空的话Parent默认会拼接moduleName
		pc.setParent("com.ciwei.mybatisplusgenerator");
		pc.setEntity("model");
		mpg.setPackageInfo(pc);

		//自定义配置
		mpg.setCfg(customerConfig(projectPath, pc));
...
//		strategy.setExclude("upms_role"); // 排除的表名
	 ...
	 */
	private static InjectionConfig customerConfig(String projectPath, PackageConfig pc) {
		InjectionConfig cfg = new InjectionConfig() {
			@Override
			public void initMap() {
				//自定义参数 可以用在模板 ${cfg.introduce}这样使用的
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("introduce","只有努力才能不被辜负");
				map.put("voPackage",pc.getParent());
				//自定义vo字段
				List<VoLists> voListsList = Lists.newArrayList();
				voListsList.add(new VoLists().setPropertyType("String").setPropertyName("jyw").setComment("测试字段"));
				voListsList.add(new VoLists().setPropertyType("String").setPropertyName("jyw2").setComment("测试字段2"));
				voListsList.add(new VoLists().setPropertyType("LocalDateTime").setPropertyName("jyw3").setComment("测试字段3"));
				map.put("voLists" ,voListsList);
				this.setMap(map);
			}
		};
		// 自定义输出配置
		...
		//设置xml
		focList.add(setXml(projectPath, pc));
		//设置vo
		focList.add(setVo(projectPath, pc));
	 ...
	 * 设置xml模板和路径
	 */
	private static FileOutConfig setXml(String projectPath, PackageConfig pc) {
		// 如果模板引擎是 freemarker
	 ...
	 * 设置vo模板和路径
	 */
	private static FileOutConfig setVo(String projectPath, PackageConfig pc) {
		String templatePath = "/templates/entityvo.java.vm";
		// 自定义配置会被优先输出
		FileOutConfig fileOutConfig = new FileOutConfig(templatePath) {
			@Override
			public String outputFile(TableInfo tableInfo) {
				// 自定义输出文件名
				return projectPath + "/src/main/java/" + pc.getParent().replace(StringPool.DOT ,StringPool.SLASH) + "/" + "vo/"
